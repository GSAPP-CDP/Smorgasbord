...
This is a jpeg (raster) version of the , but what's nice about the PDF is it has vector geometry which we can import directly into Rhino. The original file can be found [here]().

I also found these three isometric drawings. We won't use these to draft from, but they could still be a useful reference. Download all the above drawings so that you can put them in your Rhino model.
...
So. Resize this drawing (again, with the `Scale` command) to make the length of the plan 13m, like it was on the site plan. How long does this make a single bay? 13 ÷ 11 is an irrational number, which seems like an unlikely choice for a module of a floor plan. But it's roughly 1.18. You can verify this by drawing a line in Rhino and seeing how long it is. **It seems much more likely that each of these bays is a much cleaner 1.2m, making the entire plan 13.2 meters long.** Rescale the plan to match this number, and you'll find that other key dimensions are now nice round numbers as well.
...
![underlays-scaled](images/12-2/underlays-scaled.png#img-left)
*Scaled Underlays*
